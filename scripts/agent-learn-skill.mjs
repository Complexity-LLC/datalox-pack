...
if (!args.id || !args.name || !args.workflow || !args.trigger || !args.description) {
  process.stderr.write("Usage: node scripts/agent-learn-skill.mjs --id <id> --name <name> --workflow <workflow> --trigger <trigger> --description <description> [--display-name <label>] [--note-path <path>] [--tag <tag>] [--json]\nThis writes a body-first SKILL.md playbook with Datalox metadata nested under metadata.datalox.\n");
  process.exit(1);
}
...
const result = await writeSkill(
  {
    ...
    notePaths: Array.isArray(args["note-path"])
      ? args["note-path"]
      : args["note-path"]
        ? [args["note-path"]]
        : [],
